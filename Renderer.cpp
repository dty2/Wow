...
void Renderer::renderDynamic(DynamicWallPaperPtr wallpaper) {
  enum { NOTEMPTY, EMPTY, FINISH } framestatus = EMPTY;
...
void Renderer::renderStatic(StaticWallPaperPtr wallpaper) {
  GLuint texID;
  // Create texture
...
void Renderer::render(WallPaperPtr wallPaperPtr) {
  LOG(INFO) << "Wallpaper info: name:" << wallPaperPtr->name
            << " type:" << wallPaperPtr->type;

  if (wallPaperPtr->type == WallPaper::STATIC) {
    renderStatic(std::static_pointer_cast<StaticWallPaper>(wallPaperPtr));
  } else {
    renderDynamic(std::static_pointer_cast<DynamicWallPaper>(wallPaperPtr));
  }
}
